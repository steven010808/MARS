...
| 방식 | 필요한 준비 |
| --- | --- |
| Runtime bundle 사용 | 전달받은 bundle을 repo 루트에 풀어서 `data/processed/manifest.json`과 `artifacts/search`, `artifacts/recsys`, `artifacts/reports`, `artifacts/registry`가 존재하게 둡니다. 가장 빠른 실행 경로이며, 검색/추천 artifact를 다시 만들지 않습니다. |
| 외부 data 폴더 재사용 | 기존 data 폴더를 그대로 두고 `MARS_HOST_DATA_DIR` 환경변수로 경로를 지정합니다. 이 경우 processed data는 재사용하지만, 현재 repo에 runtime artifact가 없으면 bootstrap이 누락된 artifact를 재생성합니다. |
...
runtime bundle을 repo 루트에 직접 풀어둔 경우에는 환경변수 없이 실행할 수 있습니다.
...
아티팩트 재생성을 피해야 하는 시연 환경에서는 `artifacts/`가 포함된 runtime bundle을 같이 전달해야 합니다.
